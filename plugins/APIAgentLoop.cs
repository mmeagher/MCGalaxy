// APIAgentLoop.cs — ReAct-loop LLM bots via OpenAI-compatible API
//
// Deploy:
//   1. Copy this file to the server's plugins/ directory
//   2. Compile manually (see README) or in-game: /compile plugin APIAgentLoop
//   3. In-game (as owner): /plugin load APIAgentLoop
//
// Usage:
...
public sealed class APIAgentLoopPlugin : Plugin
{
    public override string name    { get { return "APIAgentLoop"; } }
    public override string creator { get { return ""; } }
    public override string welcome { get { return "APIAgentLoop loaded. Address bots with !BotName <message>"; } }
    ...
    static string LoadApiKey() {
        if (!File.Exists(KeyFile)) {
            Logger.Log(LogType.Warning, "APIAgentLoop: key file not found at " + KeyFile);
            return "";
        }
    ...
    // Keyed by bot.name
    readonly Dictionary<string, List<ApiMsg>> histories =
        new Dictionary<string, List<ApiMsg>>(StringComparer.OrdinalIgnoreCase);
    ...
    void RunLoop(Player sender, PlayerBot bot, string userText) {
        try {
            List<ApiMsg> history = GetOrCreateHistory(bot);

            lock (history) {
                history[0] = new ApiMsg("system", BuildSystemPrompt(bot));
                history.Add(new ApiMsg("user",
            ...
            for (int step = 1; step <= MaxSteps; step++) {
                ...
                // --- Think ---------------------------------------------------
                List<ApiMsg> snapshot;
                lock (history) { snapshot = new List<ApiMsg>(history); }

                string requestJson = BuildRequest(ApiModel, snapshot);
                ...
                if (string.IsNullOrEmpty(reply)) {
                    Logger.Log(LogType.Warning, "APIAgentLoop: empty response at step " + step);
                    break;
                }

                lock (history) {
                    history.Add(new ApiMsg("assistant", reply));
                    while (history.Count > HistoryLimit + 1)
                        history.RemoveAt(1);
                }

                // --- Act -----------------------------------------------------
                Logger.Log(LogType.Warning, "APIAgentLoop step " + step + " raw reply: " +
                    reply.Replace("\n", "\\n").Substring(0, Math.Min(600, reply.Length)));
                ReplyResult result = ProcessReply(bot, reply);
                Logger.Log(LogType.Warning, string.Format(
                    "APIAgentLoop step {0} result: placed={1} continue={2} done={3}",
                ...
                lock (history) {
                    history.Add(new ApiMsg("user", obs));
                }
            }

        } catch (AggregateException aex) {
            Exception inner = aex.InnerException ?? aex;
            Logger.Log(LogType.Warning, "APIAgentLoop error: " + inner.Message);
            ...
            Logger.Log(LogType.Warning, "APIAgentLoop error: " + ex.Message);
    ...
    List<ApiMsg> GetOrCreateHistory(PlayerBot bot) {
        lock (histories) {
            List<ApiMsg> history;
            if (!histories.TryGetValue(bot.name, out history)) {
                history = new List<ApiMsg>();
                history.Add(new ApiMsg("system", BuildSystemPrompt(bot)));
                ...
                history.Add(new ApiMsg("assistant", string.Format(
    ...
    // =========================================================================
    // Minimal JSON helpers (OpenAI-compatible chat completions format)
    // =========================================================================

    string BuildRequest(string model, List<ApiMsg> msgs) {
        var sb = new StringBuilder();
    ...
    // Falls back to Ollama-style {"message":{"content":"..."}} parsing
    string ParseContent(string json) {
        // Try OpenAI format: look for "choices" then "content"
        ...
        if (choicesIdx >= 0) {
            ...
            if (contentIdx >= 0) {
                ...
                if (start > 0) {
                    ...
                    if (end >= 0) return JsonUnescape(json.Substring(start, end - start));
                }
            }
        }
        // Fallback: first "content" field found (works for Ollama-style responses too)
        int idx = json.IndexOf("\"content\":");
        if (idx < 0) return null;
        int s = json.IndexOf('"', idx + 10) + 1;
        if (s <= 0) return null;
        int e = FindStringEnd(json, s);
        if (e < 0) return null;
        return JsonUnescape(json.Substring(s, e - s));
    }
    ...
    sealed class ApiMsg {
        ...
        public ApiMsg(string role, string content) { Role = role; Content = content; }
    }
}
